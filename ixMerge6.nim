...
type
  ...
  Index = object                        # Inverted Index
    ...
    inv: Table[string, HashSet[Doc]]    # map term -> docId, freqInDoc

proc names*(ix: auto, docs: auto, prefix=""): string =
  join(collect(for d in docs: prefix & ix.names[d]), "\n")

proc hash(x: Doc): Hash = hash(x.id)    # Two procs for HashSet[Doc]
proc `==`(a, b: Doc): bool = a.id == b.id
let noDocs = initHashSet[Doc](0)        # convenience empty set
...
proc initIndex*(f: File, nDigit=3, nHisto=1): Index =
  for (name, text) in f.lenPrefixedPairs(nDigit):
    ...
    for term, cnt in freq:
      ...
      result.inv.mgetOrPut(term, noDocs).incl doc
